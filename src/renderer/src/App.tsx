...
function App(): JSX.Element {
  ...
  const activeLabel = navItems.find(n => n.id === activeView)?.label ?? activeView

  return (
    <div className="flex h-screen overflow-hidden font-sans bg-background text-on-surface">
      <aside className="w-52 flex-shrink-0 border-r border-outline-variant/40 flex flex-col py-5 px-3" style={{ backgroundColor: 'rgb(var(--fp-surface-container-low))' }}>
        <div className="mb-6 px-3">
          <h1 className="text-base font-semibold text-primary tracking-tight">{activeLabel}</h1>
        </div>
